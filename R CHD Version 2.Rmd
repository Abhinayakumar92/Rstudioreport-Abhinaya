...
```{r}
CHD_tidy <- CHD_tidy %>% mutate(period = gsub(" financial years; 3-year aggregates","", period )) %>% mutate(period = gsub("to","-", period ))
```
...
```{r plot-graph, fig.width=08}
ggplot(CHD_tidy, aes(x = year, y = HospRate_100k)) + 
  labs(title = "CHD Hospitalisation Rate in Scotland (2017-2022)", 
...
ggplot(CHD_tidy, aes(x= str_wrap(period, 10), y= HospRate_100k)) + geom_col(fill = "blue4") + labs(title = "CHD Hospitalisation Rate in Scotland (2017-2022)", 
       x = "Year (3 year rolling average)", 
